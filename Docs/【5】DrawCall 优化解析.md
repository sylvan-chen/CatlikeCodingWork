...
具体到代码中，我们通过一对宏 `CBUFFER_START(_CustomName)` 和 `CBUFFER_END` 来包裹全局变量的定义。这对宏是在告诉 GPU：我要建一个新的常量缓冲区，名字叫 `_CustomName`。

常量缓冲区建立之后，CPU 端的 C# 脚本通过 `Shader.SetGlobalXXX()` 每帧传递一次。

### 核心规范：根据更新频率划分 Cbuffer

在现代图形 API（DX12, Vulkan, Metal）中，向 GPU 上传和更新 Constant Buffer 是有性能开销的。图形学中的一个黄金法则是：**把更新频率相同的变量放在同一个 Cbuffer 里。**

想象一下，如果你把「全局太阳光颜色（每帧可能只变一次）」和「材质的粗糙度（每个物体都不一样）」塞进同一个 Cbuffer：每次渲染不同物体时，为了更新粗糙度，GPU 不得不把太阳光颜色也重新上传一遍，这就造成了带宽浪费。

因此，合理的 Cbuffer 划分通常是：

- **按帧更新 (Per Frame):** 相机位置、时间、屏幕分辨率等。
- **按光源更新 (Per Light):** 光源颜色、位置、阴影矩阵。
- **按材质更新 (Per Material):** 反照率颜色、粗糙度等（`UnityPerMaterial`）。
- **按物体更新 (Per Draw):** 物体的 Transform 矩阵（`UnityPerDraw`）。
- **...**
...
2. **`cbuffer UnityPerDraw` (逐物体缓冲区)**：只放物体参数（世界坐标矩阵等）。

`UnityPerMaterial` 和 `UnityPerDraw` 是内部保留的两个特殊名字，Unity 通过找这两个名字的常量缓冲区，来判断哪些 shader 要应用 SRP Batcher。
