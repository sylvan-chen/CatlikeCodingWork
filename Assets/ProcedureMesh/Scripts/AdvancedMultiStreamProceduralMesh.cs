...
public class AdvancedMultiStreamProceduralMesh : MonoBehaviour
{
    private void OnEnable()
    {
        ...
        // 进阶程序化网格：
        // 使用之前的简单 API 向网格体分配数据时，Unity 必须在某个时刻将所有数据复制并转换为网格体的原生内存格式
        // 而使用 MeshData 能够直接以网格体的原生内存格式进行操作，避免转换
        ...
        // 分配网格的顶点数据流
        // 内存分布 (4个顶点 x 4类属性): [P][P][P][P][N][N][N][N][T][T][T][T][UV][UV][UV][UV]
        meshData.SetVertexBufferParams(vertexCount, vertexAttributes);
        // 分配给网格后记得 Dispose 原生数组
        vertexAttributes.Dispose();

        // 按顺序读每个顶点数据的内存块（每个数据块里面 4 个顶点）
        // 位置
